...
# gathering data
files = glob.glob("preprocessed_data/*.xlsx")
...
torch.save(test_data, f'dataset/torch-data/raw_test_data.pt') # save raw data
train_data.dropna(inplace=True)
train_data.drop(columns=['日期'], inplace=True)
test_data.dropna(inplace=True)
test_data.drop(columns=['日期'], inplace=True)

# feature scaling(whole dataset)
'''
# minmax scaler
...
for i in trange(train_data['股票代號'].unique().shape[0]):
    for index in range(train_data.loc[(train_data['股票代號'] == train_data['股票代號'].unique()[i])].shape[0] - seq_len):
        ...
        stock_number = torch.tensor(np.array(int(train_data['股票代號'].unique()[i]),dtype=np.float32))
        train_valid_stock_number.append(stock_number)
...
train_valid_stock_number = torch.tensor(train_valid_stock_number)
print('Shape of the train valid stock number', train_valid_stock_number.shape)
...
train_stock_number = train_valid_stock_number[:int(0.8*len(train_valid_stock_number ))]
valid_stock_number = train_valid_stock_number[int(0.8*len(train_valid_stock_number )):] 


torch.save(train_input, f'dataset/torch-data/train_input.pt')
torch.save(train_label, f'dataset/torch-data/train_label.pt')
torch.save(valid_input, f'dataset/torch-data/valid_input.pt')
torch.save(valid_label, f'dataset/torch-data/valid_label.pt')
torch.save(train_stock_number, f'dataset/torch-data/train_stock_number.pt')
torch.save(valid_stock_number, f'dataset/torch-data/valid_stock_number.pt')
torch.save(test_input, f'dataset/torch-data/test_input.pt')
torch.save(test_label, f'dataset/torch-data/test_label.pt')
torch.save(test_stock_number, f'dataset/torch-data/test_stock_number.pt')